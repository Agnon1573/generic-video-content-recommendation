...
def getData(content_val):
    ...
    """由于从 https://movie.douban.com/review/best/ 获取数据时，会遇到“如果评论剧透则不会显示”从而使得获取到的值为空的情况，因此，
    将其空值手动进行赋值。图示说明：http://img.vim-cn.com/56/b8a707fb0fcc9b4bc08c738d169f57409a36ce.png
    """
    ...
    for flag in range(0, 10):
        result = [datalist_comment_title[flag].get_text().encode('utf-8'), '\t', datalist_title[flag].get_text().encode('utf-8'), '\t', \
        datalist_url[flag], '\t', datalist_rank[flag].encode('utf-8'), '\t', list_result[flag], '\t', datalist_commentURL[flag], '\t', \
        datalist_image[flag].img["src"]]

        SQL_dataText = './dataText.txt'
        try:
